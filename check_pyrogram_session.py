#!/usr/bin/env python3
"""
Pyrogram Session Diagnostic Tool

This script checks a Pyrogram session file and displays its structure
...
def check_pyrogram_session(session_path):
    """Check a Pyrogram session file and display its structure"""
    print(f"Checking Pyrogram session: {session_path}")
    ...
    try:
        ...
        with sqlite3.connect(session_path) as conn:
            ...
            if auth_key_index is not None:
                ...
                print(f"\nAuth key found: {type(auth_key).__name__}, length: {len(auth_key) if auth_key else 0}")
...
def main():
    """Main function"""
    if len(sys.argv) < 2:
        print("Usage: python check_pyrogram_session.py <path_to_session_file>")
        return 1
    ...
    check_pyrogram_session(session_path)
